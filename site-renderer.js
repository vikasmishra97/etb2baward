(function(){
  ...
  function render(container,a,state,opts={}){
    if(!container)return;
    const cta=ctaFor(a||{}), heroImage=state.header.desktopImage||'', mobileImage=state.header.mobileImage||'';
    ...
    const navInner=`<div class="public-nav-primary"><a class="public-brand" href="website-preview.html"><img src="${esc(logo)}" alt="Logo"><span><b>${esc(a?.name||'ETB2B Awards')}</b><small>ETB2B Awards</small></span></a>${sponsor}<div class="public-nav-awards-pill"><span>ET B2B</span><b>Awards</b></div>${twoRow?'':`<nav class="public-nav-inline">${navLinks}${rewardLink}</nav>`}<a class="nav-cta ${cta.mode==='closed'?'disabled':''}" href="${cta.mode==='nominate'?'nominate.html':cta.mode==='closed'?'#':'#interest'}" data-hero-cta>${esc(cta.label)}</a></div>${twoRow?`<div class="public-nav-secondary"><nav>${navLinks}${rewardLink}</nav></div>`:''}`;
    container.innerHTML=`<div class="public-site preset-${esc(state.theme.preset)} font-${esc(state.theme.font)} radius-${esc(state.theme.radius)} ${opts.builder?'builder-site-preview':''}" style="${vars.join(';')}">${brandStrip}<header class="public-nav nav-${esc(navStyle)} ${twoRow?'two-row':'one-row'} ${state.nav.sticky?'sticky':''}">${navInner}</header><section class="public-hero hero-${esc(state.header.design)} hero-pos-${heroPos}" id="home"><div class="hero-overlay"></div><div class="hero-content"><div class="hero-copy"><small class="hero-kicker">${esc((a?.eventCategory||'ETB2B AWARDS').toUpperCase())}</small>${heroIdentity}<p>${esc(a?.description||'Recognising excellence, innovation and measurable impact.')}</p><div class="hero-meta"><span><i>📍</i><b>${esc([a?.venue,a?.city].filter(Boolean).join(', ')||'Venue to be announced')}</b></span><span><i>◷</i><b>${esc(a?.hasEventDates?niceDate(a.eventStart):'Event date to be announced')}</b></span>${a?.hasNominationDates?`<span><i>◆</i><b>Nominations: ${esc(niceDate(a.nominationStart))} – ${esc(niceDate(a.nominationEnd))}</b></span>`:''}</div></div><div id="interest" class="hero-form-wrap">${heroForm(state,cta)}</div></div></section><main class="public-main">${visibleSections.map(s=>renderSection(s,state)).join('')}</main><section class="public-final-cta"><small>ETB2B AWARDS</small><h2>${esc(cta.mode==='nominate'?'Ready to make your work count?':'Be part of the next edition')}</h2><p>${esc(cta.sub)}</p><a href="${cta.mode==='nominate'?'nominate.html':cta.mode==='closed'?'#':'#interest'}" data-hero-cta>${esc(cta.label)} →</a></section><footer class="public-footer"><span>© ${new Date().getFullYear()} ${esc(a?.name||'ETB2B Awards')}</span>${bottomSponsor}<span>Powered by ETB2B Awards · Vikas Mishra</span></footer></div>`;

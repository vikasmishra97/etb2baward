(function(){
  ...
  function defaultState(a){
    ...
    return{
      ...
      form:{showOnBanner:true,displayMode:'banner',displayModeUserSet:false,layout:'card',title:'Register your interest',subtitle:'Stay informed about nominations',fields:{name:true,email:true,mobile:true,company:true,designation:false},customFields:[],featuredButtons:[]},
  ...
  function sectionHead(s,kicker){const sub=s.showSubTitle===false?'':`<small>${esc(kicker||s.subTitle||s.label)}</small>`;const title=s.showTitle===false?'':`<h2>${esc(s.title||s.label)}</h2>`;if(!sub&&!title)return'';return `<div class="public-section-head">${sub}${title}</div>`}
  ...
  function safeExternalDesignUrl(value){const raw=String(value||'').trim();if(!/^https?:\/\//i.test(raw))return'';try{const u=new URL(raw);return (u.protocol==='https:'||u.protocol==='http:')?u.href:''}catch(e){return''}}
  function externalDesignEmbedUrl(value){const url=safeExternalDesignUrl(value);if(!url)return'';try{const u=new URL(url);const host=u.hostname.toLowerCase();if(host==='figma.com'||host.endsWith('.figma.com'))return `https://www.figma.com/embed?embed_host=share&url=${encodeURIComponent(url)}`;return url}catch(e){return''}}
  function renderExternalDesignSection(s,id){const embed=externalDesignEmbedUrl(s.customDesignUrl);if(!embed)return'';return `<section id="${id}" class="${sectionClass(s)} custom-external-section"><div class="public-custom-design-frame"><iframe src="${esc(embed)}" title="${esc((s.title||s.label||'Custom')+' custom design')}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div></section>`}
  function renderSection(s,state){if(!s.enabled)return'';const id=`section-${s.id}`;if(s.theme==='external-custom'){const custom=renderExternalDesignSection(s,id);if(custom)return custom}
  ...
  function registrationFormMarkup(state,cta,extraClass=''){
    ...
    const formMode=cta.mode==='closed'?'interest':cta.mode;
    const submitLabel=cta.mode==='closed'?'Express Interest':cta.label;
    return `<form class="hero-register form-layout-${esc(state.form.layout||'card')} ${extraClass}" data-public-interest-form data-cta-mode="${formMode}" data-capture-source="${esc(submitLabel)}"><div class="hero-register-title"><small>${formMode==='nominate'?'NOMINATIONS ARE OPEN':'STAY IN THE LOOP'}</small><b>${esc(formMode==='nominate'?'Start your nomination':state.form.title)}</b><span>${esc(state.form.subtitle||(cta.mode==='closed'?'Nominations are closed, but you can register your interest for updates.':cta.sub))}</span></div><div class="hero-register-fields">${fields}</div><button type="submit">${esc(submitLabel)} <span>→</span></button><small class="hero-consent">By continuing, you agree to receive award-related updates.</small></form>`;
  }
  ...
  function featuredButtonsMarkup(state,cta){
    const buttons=(state.form.featuredButtons||[]).filter(x=>x.active!==false&&String(x.label||'').trim());
    if(!buttons.length)return '';
    const mode=effectiveFormMode(state,cta);
    return `<div class="hero-featured-buttons">${buttons.map((b,i)=>{
      const label=esc(b.label||`Button ${i+1}`);
      if((b.action||'registration')==='registration'){
        if(mode==='banner')return `<a class="hero-featured-btn" href="#interest" data-registration-source="${label}">${label}<span>→</span></a>`;
        return `<button type="button" class="hero-featured-btn" data-open-interest-popup data-registration-source="${label}">${label}<span>→</span></button>`;
      }
      const url=String(b.url||'').trim();
      if(!url)return `<span class="hero-featured-btn disabled" aria-disabled="true">${label}<span>→</span></span>`;
      return `<a class="hero-featured-btn" href="${esc(url)}"${b.newTab?' target="_blank" rel="noopener noreferrer"':''}>${label}<span>→</span></a>`;
    }).join('')}</div>`;
  }
  function heroForm(state,cta){
    const mode=effectiveFormMode(state,cta),featured=featuredButtonsMarkup(state,cta);
    if(cta.mode==='nominate'&&mode==='banner')return registrationFormMarkup(state,cta)+featured;
    if(cta.mode==='nominate'&&mode==='popup')return `<div class="hero-popup-trigger"><small>NOMINATIONS ARE OPEN</small><b>Start your nomination</b><p>${esc(state.form.subtitle||cta.sub)}</p><button type="button" data-open-interest-popup data-registration-source="Nominate Now">${esc(cta.label)} <span>→</span></button>${featured}</div>`;
    if(cta.mode==='nominate'&&mode==='cta')return `<div class="hero-cta-only"><a class="public-primary-cta" href="#nomination-registration" data-hero-cta data-open-interest-popup data-registration-source="Nominate Now">${esc(cta.label)}</a><small>${esc(cta.sub)}</small>${featured}</div>`;
    if(cta.mode==='closed')return `<div class="hero-cta-only hero-closed-interest"><div class="hero-closed-state"><small>NOMINATIONS CLOSED</small><b>Nominations Closed</b><p>${esc(cta.sub)}</p></div><button type="button" class="public-primary-cta" data-open-interest-popup data-registration-source="Express Interest">Express Interest <span>→</span></button>${featured}</div>`;
    if(mode==='popup')return `<div class="hero-popup-trigger"><small>READY WHEN YOU ARE</small><b>${esc(state.form.title||'Register your interest')}</b><p>${esc(state.form.subtitle||cta.sub)}</p><button type="button" data-open-interest-popup data-registration-source="Express Interest">${esc(cta.label)} <span>→</span></button>${featured}</div>`;
    if(mode==='cta')return `<div class="hero-cta-only"><a class="public-primary-cta" href="#interest-popup" data-open-interest-popup data-registration-source="Express Interest">${esc(cta.label)}</a><small>${esc(cta.sub)}</small>${featured}</div>`;
    return registrationFormMarkup(state,cta)+featured;
  }
  function registrationPopup(state,cta){
    const mode=effectiveFormMode(state,cta);
    const featuredNeedsPopup=(state.form.featuredButtons||[]).some(x=>x.active!==false&&(x.action||'registration')==='registration');
    const shouldRender=(['popup','cta'].includes(mode)&&(cta.mode==='nominate'||cta.mode==='interest'||cta.mode==='closed'))||(cta.mode==='closed')||(featuredNeedsPopup&&mode!=='banner');
    if(!shouldRender)return'';
    const popupCta=cta.mode==='closed'?Object.assign({},cta,{mode:'interest',label:'Express Interest',sub:'Register your interest and we will keep you updated about the next nomination opportunity.'}):cta;
    return `<div class="public-form-modal" data-interest-popup aria-hidden="true"><div class="public-form-backdrop" data-close-interest-popup></div><section class="public-form-dialog"><button type="button" class="public-form-close" data-close-interest-popup aria-label="Close registration form">×</button>${registrationFormMarkup(state,popupCta,'popup-registration-form')}</section></div>`;
  }
  ...
  function themeVars(state){
    ...
    const fontMap={inter:'Inter,Arial,sans-serif',system:'system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif',georgia:'Georgia,Times New Roman,serif',arial:'Arial,Helvetica,sans-serif'};
    const bodyBg=state.theme.bodyBg||'#ffffff',bodyText=state.theme.bodyText||'#22252e',bodyFont=fontMap[state.theme.bodyFont]||fontMap.inter;
    const vars=[`--p:${esc(state.theme.primary)}`,`--a:${esc(state.theme.accent)}`,`--surface:${esc(state.theme.surface)}`,`--overlay:${Number(state.header.overlay||58)/100}`,`--illustration-size:${Number(state.header.illustrationSize||48)}%`,`--branding-width:${Math.max(30,Math.min(100,Number(state.header.brandingWidth||84)))}%`,`--hero-min-height:${Math.max(540,Math.min(920,Number(state.header.heroHeight||690)))}px`,`--nav-color:${esc(navColor)}`,`--nav-opacity:${navOpacity/100}`,`--nav-text:${esc(navText)}`,`--body-bg:${esc(bodyBg)}`,`--body-text:${esc(bodyText)}`,`--body-font:${bodyFont}`,`--body-heading-size:${Math.max(32,Math.min(72,Number(state.theme.bodyHeadingSize||50)))}px`,`--body-subheading-size:${Math.max(10,Math.min(24,Number(state.theme.bodySubheadingSize||16)))}px`,`--body-paragraph-size:${Math.max(12,Math.min(24,Number(state.theme.bodyParagraphSize||16)))}px`,`--body-align:${['left','center','right'].includes(state.theme.bodyTextAlign)?state.theme.bodyTextAlign:'left'}`,`--body-section-spacing:${Math.max(36,Math.min(120,Number(state.theme.bodySectionSpacing||70)))}px`];
    const heroImage=state.header.desktopImage||'',mobileImage=state.header.mobileImage||'',bodyImage=state.theme.bodyBackgroundImage||'';
    if(heroImage)vars.push(`--hero-image:url('${heroImage.replace(/'/g,"%27")}')`);if(mobileImage)vars.push(`--hero-image-mobile:url('${mobileImage.replace(/'/g,"%27")}')`);if(bodyImage)vars.push(`--body-bg-image:url('${bodyImage.replace(/'/g,"%27")}')`);
    return vars.join(';');
  }
  ...
  function navigationMarkup(a,state,cta,opts={}){
    ...
    const compact=state.nav.sticky?`<div class="public-nav-compact" data-public-nav-compact aria-hidden="true"><nav aria-label="Sticky award navigation">${navLinks}</nav></div>`:'';
    return `<header class="public-nav nav-${esc(navStyle)} ${twoRow?'two-row':'one-row'} ${state.nav.sticky?'sticky':''}" data-public-nav>${inner}</header>${compact}`;
  }
  function bindNavigation(root,opts={}){
    const nav=root?.querySelector?.('[data-public-nav]');
    const compact=root?.querySelector?.('[data-public-nav-compact]');
    if(!nav||opts.builder)return;
    let ticking=false,revealAt=72;
    const measure=()=>{
      // Measure once from the actual full header. The compact bar is fixed and never
      // changes document height, which removes the scroll jump/lag of the old collapsing nav.
      revealAt=Math.max(56,(nav.offsetTop||0)+(nav.offsetHeight||72)-6);
    };
    const closeOtherMenus=(scope,current)=>{
      scope?.querySelectorAll?.('details.public-more[open]').forEach(x=>{if(x!==current)x.open=false});
    };
    const wireMenus=scope=>scope?.querySelectorAll?.('details.public-more').forEach(d=>{
      d.addEventListener('toggle',()=>{if(d.open){closeOtherMenus(root,d)}});
    });
    const update=()=>{
      const y=window.scrollY||document.documentElement.scrollTop||0;
      const show=!!compact && nav.classList.contains('sticky') && y>=revealAt;
      if(compact){
        compact.classList.toggle('is-visible',show);
        compact.setAttribute('aria-hidden',show?'false':'true');
      }
      ticking=false;
    };
    measure();
    wireMenus(nav);wireMenus(compact);
    window.addEventListener('scroll',()=>{if(!ticking){requestAnimationFrame(update);ticking=true}},{passive:true});
    window.addEventListener('resize',()=>{measure();if(!ticking){requestAnimationFrame(update);ticking=true}},{passive:true});
    update();
  }
  ...
  function bindRegistration(root,opts={}){
    ...
    const setCaptureSource=(source)=>{if(!source)return;root?.querySelectorAll?.('[data-public-interest-form]').forEach(form=>{form.dataset.captureSource=source})};
    const open=(source)=>{if(!popup)return;setCaptureSource(source);popup.classList.add('open');popup.setAttribute('aria-hidden','false');if(!opts.builder)document.body.classList.add('public-modal-open')};
    ...
    root?.querySelectorAll?.('[data-open-interest-popup]').forEach(btn=>btn.addEventListener('click',e=>{e.preventDefault();open(btn.dataset.registrationSource||btn.textContent.trim())}));
    root?.querySelectorAll?.('[data-registration-source][href="#interest"]').forEach(btn=>btn.addEventListener('click',()=>setCaptureSource(btn.dataset.registrationSource||btn.textContent.trim())));

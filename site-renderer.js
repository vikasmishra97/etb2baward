(function(){
  ...
  function defaultState(a){
    ...
    return{
      ...
      nav:{sticky:true,showRewards:true,showContact:true,style:'transparent',background:'#4b0d13',opacity:88,textColor:'#ffffff',twoRow:true,pages:{home:true,categories:true,jury:true,guidelines:true,criteria:true,terms:true,faq:true,contact:true,previous:true},visible:{overview:true,keypoints:true,who:false,why:false,eventDescription:false,speakers:true,agenda:true,resources:false,glimpse:false,contact:true}},
  ...
  function navigationMarkup(a,state,cta,opts={}){
    ...
    const current=opts.currentPage||'home', pages=state.nav.pages||{};
    const link=(key,href,label)=>pages[key]===false?'':`<a class="${current===key?'active':''}" href="${href}">${label}</a>`;
    const mainLinks=[link('home','website-preview.html','Home'),link('categories','nominate.html','Categories'),link('jury','jury.html','Jury'),link('guidelines','entry-guidelines.html','Entry Guidelines'),link('criteria','judging-criteria.html','Judging Criteria'),link('terms','terms.html','T&C')].join('');
    const moreItems=[pages.faq===false?'':`<a class="${current==='faq'?'active':''}" href="faq.html">FAQs</a>`,pages.contact===false?'':`<a class="${current==='contact'?'active':''}" href="contact.html">Contact Us</a>`,pages.previous===false?'':`<a class="${current==='previous'?'active':''}" href="previous-edition.html">Previous Edition</a>`,state.pages.rewards?.enabled&&state.nav.showRewards!==false?`<a class="${current==='rewards'?'active':''}" href="rewards.html">Rewards</a>`:''].filter(Boolean).join('');
    const more=moreItems?`<details class="public-more"><summary>More <svg viewBox="0 0 20 20" aria-hidden="true"><path d="m6 8 4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg></summary><div>${moreItems}</div></details>`:'';
    const navLinks=mainLinks+more;
    const navStyle=state.nav.style||'transparent',twoRow=state.nav.twoRow!==false;
    const ctaHref=cta.mode==='nominate'?'nominate.html':cta.mode==='closed'?'#':(current==='home'?'#interest':'website-preview.html#interest');
  ...
  function bindNavigation(root,opts={}){
    ...
    const update=()=>{const y=window.scrollY;nav.classList.toggle('is-condensed',y>96);if(y<=96)nav.classList.remove('is-away');else if(y>lastY+5)nav.classList.add('is-away');else if(y<lastY-5)nav.classList.remove('is-away');lastY=y;ticking=false};
  ...
  window.ETB2BSite={defaultState,normalizeState,render,renderNav,navigationMarkup,themeVars,bindNavigation,ctaFor,niceDate,niceTime,lines,uid};
})();

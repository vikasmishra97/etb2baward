(function(){
  ...
  function sanitizeImportedHtml(value){const tpl=document.createElement('template');tpl.innerHTML=String(value||'').replace(/<script[\s\S]*?<\/script\s*>/gi,'');tpl.content.querySelectorAll('*').forEach(el=>{[...el.attributes].forEach(a=>{const n=a.name.toLowerCase(),v=String(a.value||'').trim().toLowerCase();if(n.startsWith('on')||((n==='href'||n==='src')&&v.startsWith('javascript:')))el.removeAttribute(a.name)})});return tpl.innerHTML}
  function sanitizeImportedCss(value){return String(value||'').replace(/<\/?style[^>]*>/gi,'').replace(/@import[^;]+;/gi,'').replace(/javascript\s*:/gi,'')}
  function customDesignMarkup(s){const d=s.customDesign||{};if(!d.enabled||!d.html)return'';const css=sanitizeImportedCss(d.css);return `<div class="custom-design-render" data-custom-design-source="${esc(d.source||'custom')}">${css?`<style>${css}</style>`:''}${sanitizeImportedHtml(d.html)}</div>`}
  function renderSection(s,state){if(!s.enabled)return'';const id=`section-${s.id}`;const custom=customDesignMarkup(s);if(custom)return `<section id="${id}" class="${sectionClass(s)} design-custom-import">${custom}</section>`;

(function(){
  ...
  function defaultState(a){
    ...
    return{
      ...
      header:{desktopImage:'',mobileImage:'',illustrationImage:'',brandingBanner:'',sponsorLogo:'',bottomSponsor:'',seoImage:'',thumbnailImage:'',seoTitle:'',seoDescription:'',design:'immersive',overlay:64,heroPosition:'center',illustrationSize:34},
  ...
  function render(container,a,state,opts={}){
    if(!container)return;const cta=ctaFor(a||{});const heroImage=state.header.desktopImage||'';const logo=a?.logoData||state.header.thumbnailImage||'et-logo.jpg';const visibleSections=(state.sections||[]).filter(s=>s.enabled);const navLinks=visibleSections.filter(s=>s.id!=='custom'&&(state.nav.visible?.[s.id]!==false)).map(s=>`<a href="#section-${s.id}">${esc(s.label)}</a>`).join('');const rewardLink=state.pages.rewards?.enabled&&state.nav.showRewards!==false?'<a href="rewards.html">Rewards</a>':'';const mobileImage=state.header.mobileImage||'';const vars=[`--p:${esc(state.theme.primary)}`,`--a:${esc(state.theme.accent)}`,`--surface:${esc(state.theme.surface)}`,`--overlay:${Number(state.header.overlay||64)/100}`,`--illustration-size:${Number(state.header.illustrationSize||34)}%`];if(heroImage)vars.push(`--hero-image:url('${heroImage.replace(/'/g,"%27")}')`);if(mobileImage)vars.push(`--hero-image-mobile:url('${mobileImage.replace(/'/g,"%27")}')`);
    const sponsor=state.header.sponsorLogo?`<div class="public-presented-by"><small>Presented by</small><img src="${esc(state.header.sponsorLogo)}" alt="Sponsor logo"></div>`:'';const bottomSponsor=state.header.bottomSponsor?`<div class="public-powered-by"><small>Powered by</small><img src="${esc(state.header.bottomSponsor)}" alt="Sponsor logo"></div>`:'';const illustration=state.header.illustrationImage?`<div class="hero-illustration"><img src="${esc(state.header.illustrationImage)}" alt="Award illustration"></div>`:'';const brandStrip=state.header.brandingBanner?`<div class="public-brand-strip"><img src="${esc(state.header.brandingBanner)}" alt="Award branding banner"></div>`:'';const heroPos=esc(state.header.heroPosition||'center');
    container.innerHTML=`<div class="public-site preset-${esc(state.theme.preset)} font-${esc(state.theme.font)} radius-${esc(state.theme.radius)}" style="${vars.join(';')}">${brandStrip}<header class="public-nav ${state.nav.sticky?'sticky':''}"><a class="public-brand" href="website-preview.html"><img src="${esc(logo)}" alt="Logo"><span><b>${esc(a?.name||'ETB2B Awards')}</b><small>ETB2B Awards</small></span></a>${sponsor}<nav>${navLinks}${rewardLink}</nav><a class="nav-cta ${cta.mode==='closed'?'disabled':''}" href="${cta.mode==='nominate'?'nominate.html':cta.mode==='closed'?'#':'#interest'}" data-hero-cta>${esc(cta.label)}</a></header><section class="public-hero hero-${esc(state.header.design)} hero-pos-${heroPos}" id="home"><div class="hero-overlay"></div>${illustration}<div class="hero-content"><div class="hero-copy"><small class="hero-kicker">${esc((a?.eventCategory||'ETB2B AWARDS').toUpperCase())}</small><h1>${esc(a?.name||'Your Award')}</h1><p>${esc(a?.description||'Recognising excellence, innovation and measurable impact.')}</p><div class="hero-meta"><span><i>📍</i><b>${esc([a?.venue,a?.city].filter(Boolean).join(', ')||'Venue to be announced')}</b></span><span><i>◷</i><b>${esc(a?.hasEventDates?niceDate(a.eventStart):'Event date to be announced')}</b></span>${a?.hasNominationDates?`<span><i>◆</i><b>Nominations: ${esc(niceDate(a.nominationStart))} – ${esc(niceDate(a.nominationEnd))}</b></span>`:''}</div></div><div id="interest" class="hero-form-wrap">${heroForm(state,cta)}</div></div></section><main class="public-main">${visibleSections.map(s=>renderSection(s,state)).join('')}</main><section class="public-final-cta"><small>ETB2B AWARDS</small><h2>${esc(cta.mode==='nominate'?'Ready to make your work count?':'Be part of the next edition')}</h2><p>${esc(cta.sub)}</p><a href="${cta.mode==='nominate'?'nominate.html':cta.mode==='closed'?'#':'#interest'}" data-hero-cta>${esc(cta.label)} →</a></section><footer class="public-footer"><span>© ${new Date().getFullYear()} ${esc(a?.name||'ETB2B Awards')}</span>${bottomSponsor}<span>Powered by ETB2B Awards · Vikas Mishra</span></footer></div>`;
  ...
  window.ETB2BSite={defaultState,normalizeState,render,ctaFor,niceDate,niceTime,lines,uid};
})();

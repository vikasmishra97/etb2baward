(function(){
  ...
  var award=safe('etb2b_awards_new_award')||{name:'India FinTech Awards 2027',slug:'india-fintech-awards-2027',currency:'INR'};
  var slug=award.slug||slugify(award.name), categoryData=safe('etb2b_awards_categories_'+slug)||[];
  var campaigns=safe('etb2b_awards_campaigns_'+slug);
  var ceremony=safe('etb2b_awards.ceremony.v16');
  var winners=safe('etb2b_awards.winners.v13');
  var shortlist=safe('etb2b_awards.shortlist.v12');
  var scoring=safe('etb2b_awards_scoring_v11');
  var audience=safe('etb2b_awards_audience_'+slug);

  function registrationRows(){
    var perAward=safe('etb2b_awards_interest_'+slug)||[], globalRows=safe('etb2b_awards_registrations')||[];
    if(!Array.isArray(perAward))perAward=[];if(!Array.isArray(globalRows))globalRows=[];
    var rows=perAward.concat(globalRows.filter(function(r){return r&&(r.awardSlug===slug||(!r.awardSlug&&r.award===award.name))}));
    var seen={},out=[];rows.forEach(function(r){if(!r||typeof r!=='object')return;var key=r.id||[(r.email||'').toLowerCase(),r.mobile||'',r.createdAt||''].join('|');if(seen[key])return;seen[key]=1;out.push(r)});
    out.sort(function(a,b){return new Date(b.createdAt||0)-new Date(a.createdAt||0)});return out;
  }
  function regDate(v){var d=new Date(v);if(isNaN(d))return{date:'Unknown',time:''};return{date:d.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}),time:d.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})}}
  function renderRegistrations(){
    ...
    var today=rows.filter(function(r){return new Date(r.createdAt||0)>=startToday}).length,week=rows.filter(function(r){return new Date(r.createdAt||0)>=weekAgo}).length,email=rows.filter(function(r){return String(r.email||'').trim()}).length;
    if($('#liveRegTotal'))$('#liveRegTotal').textContent=fmt(rows.length);if($('#liveRegToday'))$('#liveRegToday').textContent=fmt(today);if($('#liveRegWeek'))$('#liveRegWeek').textContent=fmt(week);if($('#liveRegEmail'))$('#liveRegEmail').textContent=(rows.length?Math.round(email/rows.length*100):0)+'%';
    ...
    var body=$('#registrationTableBody');if(!body)return;if(!rows.length){body.innerHTML='<tr><td class="rp-registration-empty" colspan="7">No registrations captured yet. Submit the public Express Interest form and refresh this report.</td></tr>';return}
    body.innerHTML=rows.slice(0,50).map(function(r){var d=regDate(r.createdAt);return '<tr><td class="reg-date"><b>'+esc(d.date)+'</b><small>'+esc(d.time)+'</small></td><td><b>'+esc(r.name||'—')+'</b></td><td>'+esc(r.email||'—')+'</td><td>'+esc(r.mobile||'—')+'</td><td>'+esc(r.company||'—')+'</td><td>'+esc(r.designation||'—')+'</td><td>'+esc(r.source||'Public website')+'</td></tr>'}).join('');
  }
  function downloadRegistrationsCsv(){var rows=registrationRows(),fixed=['createdAt','name','email','mobile','company','designation','source'],extra=[];rows.forEach(function(r){Object.keys(r||{}).forEach(function(k){if(fixed.indexOf(k)<0&&['id','award','awardSlug','page','status'].indexOf(k)<0&&extra.indexOf(k)<0)extra.push(k)})});var cols=fixed.concat(extra),csv=[cols].concat(rows.map(function(r){return cols.map(function(k){return r[k]==null?'':r[k]})})).map(function(r){return r.map(function(v){return '"'+String(v).replace(/"/g,'""')+'"'}).join(',')}).join('\r\n'),blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=slug+'-website-registrations.csv';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast('Registration CSV downloaded')}
  ...
  if($('#reportTitle'))$('#reportTitle').value=(award.name||'ETB2B Awards Award')+' · Executive Report';
  ...
  window.addEventListener('storage',function(e){if(e.key==='etb2b_awards_interest_'+slug||e.key==='etb2b_awards_registrations')renderRegistrations()});
  window.addEventListener('focus',renderRegistrations);
  ...
  if($('#insightAction'))$('#insightAction').addEventListener('click',function(){localStorage.setItem('etb2b_awards_selected_audience',JSON.stringify({award:slug,segmentKey:'registered',segmentName:'Registered, no entry',count:1590,createdAt:new Date().toISOString()}));window.location.href='audience.html'});
  $$('[data-recovery]').forEach(function(b){b.addEventListener('click',function(){var map={registered:['registered','Registered, no entry',1590],incomplete:['incomplete','Entry started, incomplete',296],payment:['payment','Submitted, payment pending',18],past:['past','Past-year entrants',2184]},x=map[b.dataset.recovery];localStorage.setItem('etb2b_awards_selected_audience',JSON.stringify({award:slug,segmentKey:x[0],segmentName:x[1],count:x[2],createdAt:new Date().toISOString()}));window.location.href='audience.html'})});
  ...
  function csvRows(){var d=activeFunnel(),rows=[['ETB2B Awards Executive Report',award.name||'Award'],['Period',$('#periodSelect')?$('#periodSelect').selectedOptions[0].text:'Full award cycle'],[],['Lifecycle','Value']];d.forEach(function(x){rows.push([x.label,x.value])});rows.push([],['Revenue','Amount'],['Entry fees',2142000],['Sponsorship',400000],['Add-ons',26000],[],['Source','Paid entries','Revenue','Conversion %']);sourceData.forEach(function(x){rows.push([x.name,x.paid,x.revenue,x.conv])});rows.push([],['Campaign','Sent','Open %','Click %','Conversions','Revenue']);campaigns.filter(function(c){return c.status==='sent'}).forEach(function(c){rows.push([c.name,c.sent,c.open,c.click,c.conversions,c.revenue])});var regs=registrationRows();rows.push([],['Website registrations',regs.length],['Registered at','Name','Email','Mobile','Company','Designation']);regs.forEach(function(r){rows.push([r.createdAt||'',r.name||'',r.email||'',r.mobile||'',r.company||'',r.designation||''])});var m=ceremonyMetrics();rows.push([],['Ceremony','Value'],['Confirmed guests',m.confirmed],['Checked in',m.checked],['Stage-ready awards',m.ready+'/'+m.total],['Run-of-show cues',m.cues]);return rows}
